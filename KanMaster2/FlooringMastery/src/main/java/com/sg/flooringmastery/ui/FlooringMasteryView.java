...
//******************************************************************************    
    ...
    public Order displayGetOrderInfo(List<StateTax> stateTaxes, List<Product> products) throws InvalidInputException, FlooringMasteryPersistenceException {
        io.print("        ENTER ORDER INFO   ");

        String stateNames = "";
        for(int i = 0; i < stateTaxes.size(); i++) {
            if(i == 0) {
                stateNames = stateTaxes.get(i).getStateName();
            } else {
                stateNames = stateNames + ", " + stateTaxes.get(i).getStateName();
            }
        }
        ...
        for(int i = 0; i < products.size(); i++) {
            if(i == 0) {
                productNames = products.get(i).getProductType();
            } else {
                productNames = productNames + ", " + products.get(i).getProductType();
            }
        }
        
        String customerName = io.readString("Please Enter Customer Name");
        String stateName = io.readState("Please Enter " + stateNames);
        String productType = io.readProduct("Please Enter " + productNames);
    ...
    public LocalDate displayDateBanner() {
        return displayDateBanner(null);
    }

    public LocalDate displayDateBanner(LocalDate existingOrderDate) {
        String prompt = "Please Enter Order Date ";
        if(existingOrderDate != null) {
            prompt = prompt + "(" + existingOrderDate + ")";
        }
        prompt = prompt + ": ";
        return io.readLocalDate(prompt, "MMddyyyy");
    }

    public String displayCustomerNameBanner() throws InvalidInputException {
        return displayCustomerNameBanner(null);
    }

    public String displayCustomerNameBanner(String existingCustomerName) throws InvalidInputException {
        String prompt = "Please Enter Customer Name ";
        if(existingCustomerName != null) {
            prompt = prompt + "(" + existingCustomerName + ")";
        }
        prompt = prompt + ": ";
        return io.readString(prompt);
    }

    public String displayStateTaxBanner() throws InvalidInputException {
        return displayStateTaxBanner(null);
    }

    public String displayStateTaxBanner(StateTax existingStateTax) throws InvalidInputException {
        String prompt = "Please Enter State Name ";
        if(existingStateTax != null) {
            prompt = prompt + "(" + existingStateTax.getStateName() + ")";
        }
        prompt = prompt + ": ";
        return io.readString(prompt);
    }

    public String displayProductNameBanner() throws InvalidInputException {
        return displayStateTaxBanner(null);
    }

    public String displayProductNameBanner(Product existingProduct) throws InvalidInputException {
        String prompt = "Please Enter Product Type ";
        if(existingProduct != null) {
            prompt = prompt + "(" + existingProduct.getProductType() + ")";
        }
        prompt = prompt + ": ";
        return io.readString(prompt);
    }

    public String displayAreaBanner() throws InvalidInputException {
        return displayStateTaxBanner(null);
    }

    public BigDecimal displayAreaBanner(BigDecimal existingArea) throws InvalidInputException {
        String prompt = "Please Enter Area ";
        if(existingArea != null) {
            prompt = prompt + "(" + existingArea + ")";
        }
        prompt = prompt + ": ";
...
//******************************************************************************
    public Order editOrderAttributesChange(LocalDate date, Order order) throws FlooringMasteryPersistenceException {
        int userChoice = 0;
        boolean writeAgain = true;
        ...
        while (writeAgain) {
            try {
                ...
                switch (userChoice) {
                    case 1:
                        LocalDate newDate = displayDateBanner(date);
                        if (!newDate.equals("")) {
                            updatedOrder.setDate(newDate);
                        }
                        break;
                    case 2:
                        String newCustomerName = displayCustomerNameBanner();
                        if (newCustomerName.equalsIgnoreCase("")) {
                            displayErrorMessage("************New Customer Name Cannot Be Blank!************");
                        } else {
                            updatedOrder.setCustomerName(newCustomerName);
                        }
                        break;
                    case 3:
                        String newStateName = displayStateTaxBanner(order.getTaxRate());
                        if (newStateName.equalsIgnoreCase("")) {
                            displayErrorMessage("************State Name Cannot Be Blank! Please Choose From the Following:  OH, PA, MI, or IN************");
                        } else {
                            StateTax stateTax = new StateTax(newStateName);
                            stateTax.setTaxRate(stateTax.getTaxRate(newStateName));                        
                            updatedOrder.setTaxRate(stateTax);
                        }
                        break;

                    case 4:
                        String newProductType = displayProductNameBanner(order.getProductType());
                        if (newProductType.equals("")) {
                            displayErrorMessage("************Product Name Cannot Be Blank! Please Choose from: Carpet, Laminate, Tile or Wood************");
                        } else {
                            Product product = new Product(newProductType);
                            product.setProductType(product.getProductName(newProductType));
                        ...
                        break;
                    case 5:
                        BigDecimal newArea = displayAreaBanner(order.getArea());
                        if (!newArea.equals("")) {
                    ...
                    case 6:
                        ...
                        break;
                    default:
                        displayErrorMessage("THIS IS NOT A VALID OPTION!!!!");
                }
