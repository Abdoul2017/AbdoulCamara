...
//******************************************************************************
    ...
    public void editOrder(LocalDate date, Order order)
            throws FlooringMasteryPersistenceException {
        loadOrder(date);
        if(!date.isEqual(order.getDate())) {
            removeOrder(date, order.getNumber());
            loadOrder(order.getDate());

            if(orderMapMemory.containsKey(order.getDate())) {
                order.setNumber(generateOrderNumber(order.getDate()));
                orderMapMemory.get(order.getDate()).put(order.getNumber(), order);
            } else {
                HashMap<Integer, Order> orderMap = new HashMap<>();
                orderMap.put(order.getNumber(), order);
                orderMapMemory.put(order.getDate(), orderMap);
            }
        } else {
            orderMapMemory.get(date).put(order.getNumber(), order);
        }
        saveOrder(order.getDate());
    }
